    ...
    public class CornerToolbarViewModel : ReactiveObject
    {
        ...
        public CornerToolbarViewModel(Func<Task> cacheAllFunc, Func<Task> launchRandomFunc, Func<bool, Task> refreshDirFunc, IDialogService dialog, TeensyStorageType storageType)
        {
            ...
            CacheAllCommand = ReactiveCommand.CreateFromTask(
                execute: async _ =>
                {
                    var confirm = await dialog.ShowConfirmation($"This will creates a local index of all the files stored on your {storageType}.\r\r This will enable search and file launch randomization to span your entire collection.\r\rThis may take a few minutes if you have thousands of files. Proceed?");
                    if (!confirm) return;
