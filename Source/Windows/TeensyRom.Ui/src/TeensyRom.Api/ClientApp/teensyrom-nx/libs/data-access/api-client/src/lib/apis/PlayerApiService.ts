...
export class PlayerApiService extends runtime.BaseAPI {
   ...
   */
  async toggleMusicRaw(
    requestParameters: ToggleMusicRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<ToggleMusicResponse>> {
    if (requestParameters['deviceId'] == null) {
      throw new runtime.RequiredError(
        'deviceId',
    ...
    const response = await this.request(
      {
        path: `/devices/{deviceId}/toggle-music`.replace(
          `{${'deviceId'}}`,
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      ToggleMusicResponseFromJSON(jsonValue)
    );
  }
